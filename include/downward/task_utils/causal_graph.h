...
/*
  ...
  domains with huge numbers of variables.
...
public:
    ...
    /* Use the factory function get_causal_graph to create causal graphs
       to avoid creating more than one causal graph per ClassicalPlanningTask. */
    explicit CausalGraph(const ClassicalPlanningTask& task);
...
/* Create or retrieve a causal graph from cache. If causal graphs are created
   with this function, we build at most one causal graph per ClassicalPlanningTask. */
extern const CausalGraph& get_causal_graph(const ClassicalPlanningTask* task);
} // namespace causal_graph
#endif
