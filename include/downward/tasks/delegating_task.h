...
/*
  ...
  the parent task. You should inherit from this class instead of ClassicalPlanningTask
  if you need specialized behavior for only some of the methods.
*/
class DelegatingTask : public ClassicalPlanningTask {
protected:
    const std::shared_ptr<ClassicalPlanningTask> parent;

public:
    explicit DelegatingTask(const std::shared_ptr<ClassicalPlanningTask>& parent);
