...
#include "downward/plugins/types.h"
...
DecoratedASTNodePtr LetNode::decorate(DecorateContext& context) const
{
    ...
    return std::make_unique<DecoratedLetNode>(
        variable_name,
        std::move(decorated_definition),
        std::move(decorated_nested_value));
}
    ...
    CollectedArguments& arguments) const
{
    ...
    if (arg_info.bounds.has_bound()) {
        ...
        decorated_arg = std::make_unique<CheckBoundsNode>(
            ...
            std::move(decorated_min_node),
            std::move(decorated_max_node));
    }
...
DecoratedASTNodePtr ListNode::decorate(DecorateContext& context) const
{
        ...
        for (size_t i = 0; i < elements.size(); i++) {
            ...
            if (element_type != *common_element_type) {
                ...
                decorated_element_node = std::make_unique<ConvertNode>(
                    std::move(decorated_element_node),
                    element_type,
                    *common_element_type);
            }
    ...
    return std::make_unique<DecoratedListNode>(std::move(decorated_elements));
}
