...
CostAdaptedTask::CostAdaptedTask(
    const shared_ptr<ClassicalPlanningTask>& parent,
    OperatorCost cost_type)
    : DelegatingTask(parent)
    , cost_type(cost_type)
    , parent_is_unit_cost(task_properties::is_unit_cost(*parent))
{
}
...
int CostAdaptedTask::get_operator_cost(int index) const
{
    return get_adjusted_action_cost(
        parent->get_operator_cost(index),
        cost_type,
        parent_is_unit_cost);
}

class CostAdaptedTaskFeature
    : public plugins::TypedFeature<ClassicalPlanningTask, CostAdaptedTask> {
public:
