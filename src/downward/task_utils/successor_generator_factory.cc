...
SuccessorGeneratorFactory::construct_fork(vector<GeneratorPtr> nodes) const
{
    ...
    if (size == 1) {
        ...
        /* This general case includes the case size == 0, which can
           (only) happen for the root for domains with no operators. */
...
static vector<FactPair> build_sorted_precondition(const AbstractOperatorProxy& op)
{
...
{
    AbstractOperatorsProxy operators = task.get_abstract_operators();
