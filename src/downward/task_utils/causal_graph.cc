...
/*
  ...
  CostAdaptedTask.
  We have the same problems for other objects that are associated with domains
...
static unordered_map<const ClassicalPlanningTask*, unique_ptr<CausalGraph>>
    causal_graph_cache;
    ...
    void handle_operator(const OperatorProxy& op)
    {
        ...
        for (FactProxy pre : op.get_preconditions()) {
            ...
            for (FactProxy eff : effects) {
                int eff_var_id = eff.get_variable().get_id();
        ...
        for (size_t i = 0; i < effects.size(); ++i) {
            int eff1_var_id = effects[i].get_variable().get_id();
            for (size_t j = i + 1; j < effects.size(); ++j) {
                int eff2_var_id = effects[j].get_variable().get_id();
...
CausalGraph::CausalGraph(const ClassicalPlanningTask& task)
{
    utils::Timer timer;
...
void CausalGraph::dump(const ClassicalPlanningTask& task) const
{
...
const CausalGraph& get_causal_graph(const ClassicalPlanningTask* task)
{
